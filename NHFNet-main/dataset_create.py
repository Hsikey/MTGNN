...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--align", action="store_true", default=True, help="If Aligned"
    )
    args = parser.parse_args()

    DATA_PATH = args.datadir
    CSD_PATH = os.path.join(DATA_PATH, "csd")
    TRAIN_PATH = os.path.join(DATA_PATH, "trainNew1")
    VAL_PATH = os.path.join(DATA_PATH, "valNew1")
    TEST_PATH = os.path.join(DATA_PATH, "testNew1")
    ...
    if args.align:
        ...
        train50, val50, test50 = dataset.get_tensors(
            seq_len=50,
            non_sequences=[label_field],
            direction=False,
            folds=[
        ...
        train375, val375, test375 = dataset.get_tensors(
            seq_len=375,
            non_sequences=[label_field],
            direction=False,
            folds=[
        ...
        train500, val500, test500 = dataset.get_tensors(
            seq_len=425,
            non_sequences=[label_field],
            direction=False,
            folds=[
    ...
    if args.align:
        ...
        print(
            "Val:",
            val[text_field].shape,
            val[visual_field].shape,
            val[acoustic_field].shape,
        )
        print(
            "Test:",
        ...
        print(
            "Val:",
            val50[text_field].shape,
            val500[visual_field].shape,
            val375[acoustic_field].shape,
        )
        print(
            "Test:",
    ...
    if args.align:
        ...
        #     [train50, val50, test50], [TRAIN_PATH, VAL_PATH, TEST_PATH]
        # ):
        #     for f, n in zip([label_field, text_field], ["label", "text"]):
        #         np.save(os.path.join(path, n + "50.npy"), split[f])


        # for split, path in zip(
        #     [train375, val375, test375], [TRAIN_PATH, VAL_PATH, TEST_PATH]
        # ):
        ...
        for split, path in zip(
            [train500, val500, test500], [TRAIN_PATH, VAL_PATH, TEST_PATH]
        ):
