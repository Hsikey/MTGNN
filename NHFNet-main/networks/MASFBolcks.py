...
if __name__ == "__main__":
    ...
    m1 = torch.rand(128, 50, 32).to(device)  # 第一个模态的特征
    m2 = torch.rand(128, 100, 32).to(device)  # 第二个模态的特征
    x = [m1, m2]
    net = MSAF([50, 100], 5, block_dropout=0.2, reduction_factor=4, split_block=5).to(device)
    y = net(x)
    print(y)
...
#     # 初始化MSAFBlock模块
#     msaf_block = MSAFBlock([50, 100], 5, block_dropout=0.2, lowest_atten=0.1, reduction_factor=4).to(device)
