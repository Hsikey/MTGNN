    ...
    def __init__(self, dataset_folder, param):
        ...
        for p in param:
            ...
            if p == "label" and "sentiment" in param[p]:
                self.data.append(
                    np.load(os.path.join(dataset_folder, p + "50.npy"))[:, :, 0]
                )
            elif p == "label" and "emotion" in param[p]:
                self.data.append(
                    np.load(os.path.join(dataset_folder, p + "50.npy"))[:, :, 1:]
                )
            else:
                self.data.append(np.load(os.path.join(dataset_folder, p + ".npy")))
